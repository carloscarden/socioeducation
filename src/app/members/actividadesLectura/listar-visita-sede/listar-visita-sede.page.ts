...
@Component({
  selector: 'app-listar-visita-sede',
  templateUrl: './listar-visita-sede.page.html',
  styleUrls: ['./listar-visita-sede.page.scss'],
})
export class ListarVisitaSedePage implements OnInit {
  url;
  restInfScroll;
  ...
  diseniarPDFcomoLoQuiereElUsuario(){
      ...
      return this.visitaService.getAllVisitas(this.inspectorId, formatoCorrectoInicio, formatoCorrectoFin, filtrado, 1000,0);
  ...
  armarPDF(visitasAllenar: Array<VisitaEscuela>){
    ...
    visitasAllenar.forEach(visita => {
      ...
      var formatoUrgentePDF = visita.urgente=="T" ? "Si" : "No";

      contenidoDeLaVisita.push(formatoInicio, formatoFin, visita.establecimiento.cue, {ul:contenidoDeLosMotivos}, visita.acompaniante, visita.observaciones, formatoUrgentePDF );
