...
@Component({
  ...
  templateUrl: './listar-gestion-territorial.page.html',
  styleUrls: ['./listar-gestion-territorial.page.scss'],
})
export class ListarGestionTerritorialPage implements OnInit {
  url;
  restInfScroll;
  ...
  constructor( private trabajosService:GestionTerritorialService, 
  ...
  diseniarPDFcomoLoQuiereElUsuario(){
    ...
    console.log("incio filtro",this.inicioFiltro);
    console.log("incio filtro",this.finFiltro);
    if(this.inicioFiltro && this.finFiltro){
        let diaInicio = this.inicioFiltro.split("-");
        formatoCorrectoInicio = diaInicio[0]+"/"+diaInicio[1]+"/"+diaInicio[2];
     
        console.log("fin filtro",this.finFiltro);
        let diaFin = this.finFiltro.split("-");
